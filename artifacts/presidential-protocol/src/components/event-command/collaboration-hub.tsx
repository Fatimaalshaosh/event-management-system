...
import { CURRENT_USER, personName, personRole, usePeopleVersion } from "@/lib/identity";
          ...
          <div className="flex gap-3.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-[13px] font-semibold text-foreground" style={{ fontFamily: "Georgia, serif" }}>{personName(m.author, lang)}</span>
                {m.kind === "ai" && <StatusPill label={L(lang, "AI", "ذكاء")} color={C.gold} />}
                <span className="text-[11px] text-muted-foreground">· {personRole(m.author, lang)}</span>
                ...
                <div className="mt-3 ps-3 space-y-2.5" style={{ borderInlineStart: `2px solid ${C.border}` }}>
                  {childReplies.map((r) => (
                    <div key={r.id} className="flex gap-2.5">
                      ...
                      <div className="min-w-0"><div className="flex items-center gap-1.5"><span className="text-[12px] font-semibold text-foreground">{personName(r.author, lang)}</span><span className="text-[10px] text-muted-foreground">{r.at}</span></div><Body text={r.body} /></div>
                    </div>
                  ))}
                </div>
              )}
...
export function EventCollaborationHub({ eventId: _eventId }: { eventId: number }) {
  ...
  usePeopleVersion(); // re-render when Contacts (canonical people) load, so names/titles refresh
